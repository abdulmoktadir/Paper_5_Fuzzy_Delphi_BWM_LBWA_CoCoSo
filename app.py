...
# ---------- Module 2: Fuzzy BWM (Revised with proper state handling) ----------
    ...
    if 'bwm_initialized' not in st.session_state:
        st.session_state['bwm_initialized'] = False
    ...
    if st.button("Start Expert Data Entry"):
        st.session_state['bwm_initialized'] = True
        st.session_state['bwm_factors'] = factors
        st.session_state['bwm_n_exp'] = n_exp
        st.session_state['bwm_expert_data'] = [None] * n_exp
        st.session_state['bwm_current_expert'] = 0
    ...
    # Data entry phase
    if st.session_state.get('bwm_initialized', False) and not st.session_state['bwm_data_entry_done']:
    ...
    # After all data entered, process and compute weights
    if st.session_state.get('bwm_data_entry_done', False):
        ...
        if not best_counts or not worst_counts:
            st.error("No valid expert data found.")
        else:
    ...
    if st.button("Reset BWM Data"):
        for key in ['bwm_initialized', 'bwm_factors', 'bwm_n_exp', 'bwm_expert_data', 'bwm_current_expert', 'bwm_data_entry_done']:
...
# ---------- Module 3: Fuzzy LBWA & Hybrid ----------
    ...
    if st.checkbox("Use same factors as in BWM"):
        if 'bwm_factors' in st.session_state:
            factors = st.session_state['bwm_factors']
            best_idx = st.session_state.get('bwm_best', 1)  # fallback
        else:
